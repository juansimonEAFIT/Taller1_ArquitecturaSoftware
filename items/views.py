...
from .repositories import get_pedido_repository
...
def pedidos_empresa(request):
    ...
    pedidos = repo.list_for_empresa(request.user.empresa, estado)

    # Cambiar estado de pedido (POST)
    if request.method == "POST":
        pedido_id = request.POST.get('pedido_id')
        nuevo_estado = request.POST.get('nuevo_estado')
        # usar repositorio para aplicar la regla
        repo.change_estado(pedido_id, request.user.empresa, nuevo_estado)
...
def pedido_recibo_pdf(request, pedido_id):
    repo = get_pedido_repository()
    try:
        pedido = repo.get_for_cliente_pdf(pedido_id, getattr(request.user, 'nombre', ''))
        if not pedido:
